...
describe(`${scenarioName} - ${module} `, () => {
  ...
  it('Add product', () => {               
    ...
    it('Search Product', () => {
      ...
      cy.get('.css-k31g74').contains('pantalon').then(data =>{
        cy.get('[data-cy="delete-501"]').click().then( a =>{
    ...
    it('verify delete product', () => {
      ...
      cy.get('[data-cy="search-type"]').select('ID')
      cy.get('[data-cy="search-bar"]').type('501{enter}' )
      cy.contains('pantalon').should('not.exist').then(() =>{
        cy.window().then((win) => {
          win.alert('**Producto eliminado exitosamente**')
